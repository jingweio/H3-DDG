...
# ---------------------------------------------------------------------------------------------
# TEMPORARY (2026-08-28, user-approved): evaluation ensemble size.
#
# Upstream scores the whole held-out fold FIVE times per epoch, each with a freshly drawn
# ProteinMPNN decoding order (model.randn = None forces a redraw; protein_mpnn_utils.py:1069-1097
# turns randn into the autoregressive decoding order), then averages. That averaging is what the
# `for _ in range(4)` + `valid_pred /= 5` below does.
#
# It also costs ~80% of the wall clock: fold 3's held-out set is 142,905 rows = 17,864 batches,
# so one epoch is 31 s of training plus five 40-minute scoring passes (~3.4 h/epoch, ~2.5 days
# for the fold). Setting this to 1 keeps a single decoding order and makes an epoch ~40 min.
#
# NOTE FOR THE RECORD: folds 1 and 2 were produced on Ibex with the upstream value of 5. Folds
# 0/3/4 run with 1, so their held-out scores carry the variance of a single decoding order rather
# than the mean of five, and -- because patience-3 selects on that same noisier number -- they may
# also select a different epoch than a 5-sample run would have. This is a reproduction check, not
# a like-for-like per-fold table.
# ---------------------------------------------------------------------------------------------
EVAL_ENSEMBLE = 1
    ...
    for fold in _folds_to_run:
        ...
        for epoch in (range(_start_epoch, epochs) if not _skip_training else []):
            ...
            if args.model_type == 'structure':
                ...
                for _ in range(EVAL_ENSEMBLE - 1):
                    valid_pred1 = []
                    model.randn = None
                    bar = tqdm(valid_dataloader)
                    for k,data in enumerate(bar):
