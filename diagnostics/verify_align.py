...
for dms, g in oof.groupby('DMS_id'):
    src = pd.read_csv(os.path.join(repo, 'data/input/Binding_substitutions_DMS', f'{dms}.csv'))
...
for dms, g in oof.groupby('DMS_id'):
    src_n = len(pd.read_csv(os.path.join(repo, 'data/input/Binding_substitutions_DMS', f'{dms}.csv')))
